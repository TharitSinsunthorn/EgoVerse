...
class YAM(Embodiment):
    ...
    def get_transform_list(
        ...
        if mode == "cartesian":
            return _build_yam_bimanual_transform_list(is_quat=True)
...
def _build_yam_bimanual_eef_frame_transform_list(
    *,
    ...
    stride: int = 1,
    extrinsics_key: str = "yamApr2026",
    is_quat: bool = True,
) -> list[Transform]:
    """YAM bimanual transform pipeline with actions expressed relative to the
...
def _build_yam_bimanual_transform_list(
    *,
    left_target_world: str = "left_extrinsics_pose",
    right_target_world: str = "right_extrinsics_pose",
    left_cmd_world: str = "left.cmd_ee_pose",
    ...
    """Canonical YAM bimanual transform pipeline used by tests and notebooks."""
    extrinsics = EXTRINSICS[extrinsics_key]
    left_extrinsics_pose = _matrix_to_xyzwxyz(extrinsics["left"][None, :])[0]
    right_extrinsics_pose = _matrix_to_xyzwxyz(extrinsics["right"][None, :])[0]
    left_extra_batch_key = {"left_extrinsics_pose": left_extrinsics_pose}
